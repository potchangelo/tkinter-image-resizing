...
# images ใช้เก็บข้อมูลรูปที่ถูกเพิ่มเข้ามา
# column_frame ใช้ระบุตำแหน่งของรูป ที่จะวางไปใน Grid
images = []
column_frame = 0

def display():
    # โหลดตัวแปรข้างนอก มาใช้ใน Function
    global images
    global column_frame

    # โหลดรูป
    path_image = fd.askopenfilename(filetypes=(('image files', '*.png *.jpg'),))
    image = Image.open(path_image)
    image_resized = image.resize((50, 50))
    image_tk = ImageTk.PhotoImage(image_resized)

    # สร้างรูปด้วย Label
    tk.Label(master=window, image=image_tk).grid(row=0, column=column_frame)

    # เก็บข้อมูลรูป และอัพเดทเลข column
...
# ปุ่ม Add image
button = tk.Button(master=window, text='Add image', command=display)
